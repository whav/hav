...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument("--api_base_url", type=str, default="http://localhost:8000",
        ...
        for line_number, line in csv_data:
            ...
            if options['retry']:
                ...
                if status_code is None:
                    raise CommandError("You provided the '--resume' flag but your CSV-data does not contain a\
csv_import_status field. Make sure you are using a tracklog of a previous import run as data source.")
                if str(status_code) == "201":
                    ...
                    print(f"Attemting to reimport {line['SourceFile']}(tracklog line numer: {line_number})…")
            ...
            target_file_node = get_or_create_subnodes_from_path(rel_file_path,
                                                                target_parent_node)
