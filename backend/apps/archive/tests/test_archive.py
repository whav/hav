...
class ArchiveTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        with patch.object(ArchiveFile, 'resolve_source', return_value=cls.test_file):
            cls.af = ArchiveFile.objects.create(source_id=str(cls.test_file), created_by=cls.user)
            archive_file(cls.af.pk)

        with patch.object(ArchiveFile, 'resolve_source', return_value=cls.attachment_file):
            cls.attachment = AttachmentFile.objects.create(source_id=str(cls.attachment_file), created_by=cls.user)
